...
@bpp.run_decorator()
def _take_radiograph_once(num_image, exposure_time, deadtime):
    ...
    def acquisition():
        # Hold the PandA start signal low while configuring.
        ...
        # Arm the camera before starting the external pulses.
        yield from bps.kickoff_all(kinetix1, wait=True)
    ...
    # Reset the start signal on completion or a plan error.
...
def take_radiograph(
    ...
    for iteration in range(num_iterations):
        yield from _take_radiograph_once(
            num_image, exposure_time, deadtime
        )
